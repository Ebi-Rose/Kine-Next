...
| Transactional email | Resend | SMTP via Supabase auth config |
| Domain | kinefit.app | Cloudflare DNS |
...
| `RESEND_API_KEY` | Resend SMTP (used as Supabase SMTP password) | Supabase Auth config |
...
3. Check Resend status page (status.resend.com)
4. If Vercel is down: nothing to do — wait for their recovery
5. If Supabase is down: app works offline (localStorage), but auth/sync fails
6. If Resend is down: signup/password reset emails won't send, but existing users can still log in
7. If our code is the issue: rollback via Vercel (see above)
8. Post status update in [wherever you communicate with users]
