...
### Decision
Use Resend as the SMTP provider for Supabase auth emails (signup confirmation, password reset).

### Context
Supabase's built-in email service has a strict rate limit (~3 emails/hour on the free tier) and returns misleading errors when the limit is hit. Production auth requires a reliable email provider.

### Reasoning
- Generous free tier (3,000 emails/month) — more than sufficient for early growth
- Simple setup — SMTP credentials plug directly into Supabase's auth config
- DNS verified via Cloudflare (MX, SPF, DKIM records on `kinefit.app`)
- Emails come from `noreply@kinefit.app` — professional and domain-verified
- Good deliverability and inbox placement out of the box

### Trade-offs
- Another vendor dependency (mitigated: easy to swap SMTP providers)
- Free tier has limits — need to upgrade if volume grows significantly

### Alternatives considered
- Supabase built-in mailer (too rate-limited for production, emails land in spam)
- SendGrid (more complex setup, overkill for current volume)
- AWS SES (cheapest at scale, but complex DNS/IAM setup for a solo team)
